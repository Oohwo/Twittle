...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func user(userId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let url = "https://api.twitter.com/1.1/users.show.json?"
        TwitterAPICaller.client?.post(url, parameters: ["screen_name" : "rs4cd4"], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in failure(error)
        })
    }
